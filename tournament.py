...
def playerStandings():
    ...
    DB = connect()
    c = DB.cursor()
    c.execute("select players.id, playerName, (select count(match.id) from\
                    match where players.id=match.playerID and score >= 2) as\
                    wins, (select count(match.id) from match where\
                    players.id=match.playerID) as matches from players\
                    group by id order by wins desc, matches")
    result = c.fetchall()
    DB.close()
    return result

    # This function is hard coded to return the count of '2' scores as wins
    # to satisfy the requirements of the testing routine without regard to
    # ties or byes.  In a more robust application the function would have
    # returned a 'score' or 'standing' as well as a match count.  In that
    # case, the first subselect would have been written as
    # (select sum(score) from match....)


def reportMatch(winner, loser, result='win'):
    """Records the outcome of a single match between two players.

    Args:
      winner:  the id number of the player who won
      loser:  the id number of the player who lost
      result:  'draw' for a tie, 'bye' for a default bye
    """
    if result == 'draw':
        p1Score = 1
        p2Score = 1
    ...
    DB = connect()
    c = DB.cursor()
    c.execute("insert into match(playerID, opponentID, score) values\
                ( %s, %s, %s);", (winner, loser, p1Score))
    c.execute("insert into match (playerID, opponentID, score) values\
                (%s,%s,%s);", (loser, winner, p2Score))
    DB.commit()
    DB.close()

    # we are inserting the values of 2 for win and 0 for lose in this case.
    # In a more robust application, we may have situations which
    # require the scoring of "draws" and "byes", which won't count for as
    # much as a true win.  We are also inserting two records into the table,
    # because each player (in this case 2) has a result from the match.  If it
    # were a Hungry Hippos game, we would insert 4 records with the number of
    # marbles for each hippo, or n records for each of n players of Monoply
...
def findWinner():
    DB = connect()
    c = DB.cursor()
    c.execute("select players.id, playerName, (select sum(score) from match)\
                as score ")
    result = c.fetchall()
    DB.close()
    return result
