...
def playerStandings():
    ...
    c.execute("select players.id, playerName, (select count(match.id) from\
                    match where players.id=match.playerID and score >= 3) as\
    ...
    # This function is hard coded to return the count of '3' scores as wins
...
def reportMatch(winner, loser):
    ...
    c.execute("insert into match(playerID, opponentID, score) values\
                ( %s, %s, %s);", (winner, loser, 3))
    c.execute("insert into match (playerID, opponentID, score) values\
                (%s,%s,%s);", (loser, winner, 0))
    DB.commit()
    DB.close()

    # we are inserting the values of 3 for win and 0 for lose in this case.
