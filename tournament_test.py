...
def testDrawMatch():
    """
    This test case is added to test reporting of a match reported as a draw 
    """
    ...
    [id1, id2, id3, id4] = [row[0] for row in players]
    reportMatch(id1, id2, 'draw')
    reportMatch(id3, id4)

    standings = playerStandings()

    if standings[0][0] != id3:
        raise ValueError("a player with a true win should be in first place")
    if standings[3][0] != id4:
        raise ValueError("a player with a true loss should be in last place")

    print "11. Players are properly ranked with wins counting more than draws"
